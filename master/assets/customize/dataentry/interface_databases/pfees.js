function ProjectInterface(record, params) {
  ApplicationInterface.call(this, record, params);
  var app_interface = this;

  var pfees_interface_params = {
    'pfees_costs_group' : 'project_fee_grp',
    'pfees_costs_container' : 'table.pfees_costs tbody',
    'pfees_costs_mnemonics' : ['proj_procedure', 'proj_hours', 'proj_staff', 'proj_fee', 'proj_proced_note'],
    'pfees_costs_data_entry_form' : '#costs_data_entry',
  ...
  **
  ...
  this.updateField = function(calling_field) {
    if (calling_field.attr('id').match(pfees_interface_params.pmnemonics_to_ignore) === null) {
  ...
  /*****
  **
  **  populateForm : Performs the same function as ApplicationInterface.populateForm, but also performs pfees record specific interface handling
  **
  **  notes:
  ...
  **
  ...
  this.populateForm = function() {
    if ($(pfees_interface_params.pfees_costs_container).length > 0) {
  ...
  **
  ...
  this.populateCosts = function() {
    ...
    $(pfees_interface_params.pfees_costs_container).find('tr').remove();
    ...
    if (record.hasCosts()) {
      ...
      for (var i = 1; i <= record.getCostsCount(); i++) {
        ...
        for (var mnemonic = 0; mnemonic < pfees_interface_params.pfees_costs_mnemonics.length - 1; mnemonic++) {
          html.append(addCostPiece(record.getCost(i), pfees_interface_params.pfees_costs_mnemonics[mnemonic]));
        }

        occurrences_html.append(html);
      }

      $(pfees_interface_params.pfees_costs_container).append(occurrences_html.children());
    } else {
      $(pfees_interface_params.pfees_costs_container).append('<tr><td colspan="5">No Costs Found</td></tr>');
    }
  };
  ...
  **
  ...
  this.selectCost = function(calling_field) {
    ...
    if ($(calling_field).hasClass('selected')) {
      $(pfees_interface_params.pfees_costs_container + ' tr').removeClass('selected');
    } else {
      $(pfees_interface_params.pfees_costs_container + ' tr').removeClass('selected');
  ...
  /*****
  **
  **  addCost : Displays a lightbox with a form for users to fill out to add a new cost to a pfees record
  **
   *****/
  this.addCost = function() {
    $(pfees_interface_params.pfees_costs_data_entry_form + ' :input').val('');
    $('#pfeescost_occurrence').val(record.getCostsCount() + 1);
  ...
  /*****
  **
  **  addCost : Displays a lightbox with a form for users to fill out to edit an existing cost in a pfees record
  **
   *****/
  this.editCost = function() {
    if ($(pfees_interface_params.pfees_costs_container + ' tr.selected').length > 0) {
      var occurrence = $(pfees_interface_params.pfees_costs_container + ' tr.selected').find('td:first').text();
      $(pfees_interface_params.pfees_costs_data_entry_form + ' :input').val('');
  ...
  /*****
  **
  **  removeCost : removes an existing cost from a pfees record if the user confirms removal
  **
   *****/
  this.removeCost = function() {
    if ($(pfees_interface_params.pfees_costs_container + ' tr.selected').length > 0) {
      ...
      if (confirm("Are you sure you want to remove this occurrence?")) {
        record.removeGroupOccurrence(record.getGroup(pfees_interface_params.pfees_costs_group, occurrence));
        app_interface.populateCosts();
  ...
  **
  ...
  this.costColorbox = function(title) {
    $.colorbox({
      inline: true,
      href: pfees_interface_params.pfees_costs_data_entry_form,
      title: title,
      transition: 'elastic',
      width: '900px',
      height: '600px',
      onCleanup: function() {
        // Clear Form Fields
        $('#' + pfees_interface_params.pfees_costs_group.toUpperCase()).find(':input').val('');
        $('#' + pfees_interface_params.pfees_costs_group.toUpperCase()).find('option:selected').removeAttr('selected');
  ...
  /*****
  **
  **  saveCost : gets values from a form lightbox and populates a pfees record with the modified (or new) costs occurrence
  **
  **  params:
  ...
  **
  ...
  this.saveCost = function(occurrence) {
    ...
    if (!record.getCost(occurrence) && record.getCostsCount() > 0) {
      // RL-2020-12-21
      record.addGroup(pfees_interface_params.pfees_costs_group, occurrence, record.getCosts(), true);
    } else if (!record.getCost(occurrence) && record.getCostsCount() <= 0) {
      // RL-2020-12-21
      record.addGroup(pfees_interface_params.pfees_costs_group, 1, null, true);
    }

    cost_group = record.getCost(occurrence);
    var costs_mnemonics = pfees_interface_params.pfees_costs_mnemonics;

    for (var i = 0; i < costs_mnemonics.length; i++) {
      if ($('#pfeescost_' + costs_mnemonics[i]).val().length > 0) {
        var mnemonic_value = $('#pfeescost_' + costs_mnemonics[i]).val();
  ...
  /*****
  **
  **  populateRight_cost Form : gets values from a passed occurrence in the pfees cost group and populates the editing form with those values
  **
  **  params:
  ...
  **
  ...
  this.populateCostsForm = function(occurrence) {
    ...
    var costs_mnemonics = pfees_interface_params.pfees_costs_mnemonics;
    $('#pfeescost_occurrence').val(occurrence);

    for (var i = 0; i < costs_mnemonics.length; i++) {
      if (record.getElement(costs_mnemonics[i], 0, cost_occurrence)) {
        $('#pfeescost_' + costs_mnemonics[i]).val(record.getElement(costs_mnemonics[i], 0, cost_occurrence).text());
      }
    }
  };
  ...
  /*****
  **
  **  calculateTotalCost : gets values from all occurrences in the pfees cost group, and all values from pfees taxes,
  ...
  **
  ...
  this.calculateTotalCost = function() {
    var total = 0;
    var costs_1 = record.getElement('prj_tax1').text();
    var costs_2 = record.getElement('prj_tax2').text();

    if (record.getGroup('project_fee_grp', 1)) {
      var costs = record.getGroup('project_fee_grp', 1).parent();

      for (var i = 0; i < $(costs).children().length; i++) {
        if ($(costs).children().eq(i).find('proj_fee').length > 0) {
          cost = parseFloat($(costs).children().eq(i).find('proj_fee').text());
      ...
      $('#PROJ_TOTAL_COST').val(total);
      $('#PROJ_TOTAL_COST').change();
    } else {
