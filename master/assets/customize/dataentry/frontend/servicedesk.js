...
function exitController(calling_field)
{
  ...
  // call 'lookup_container' class user routine
  if ( $(calling_field).hasClass('lookup_container') ) {
    var result = lookupContainer(calling_field);
    if ( !result ) {
      alert ( '**Warning** the selected item is restricted for access' );
    }
  }

  // RL-20220913
  // call 'get_item_info' class user routine
  if ( $(calling_field).hasClass('get_item_info') ) {
    var result = getItemInfo(calling_field);
  }
