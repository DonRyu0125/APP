/*
  ...
  24)  gen_set_cond_help - show sub-menu values according to the main menu value
       parameters: data-main=<mnemonic of primary key of main menu database>
                   data-sub-source=<mnemonic of alternal source field"
  ...
  27) manip_record - add or change record of other database inside the data entry form
      parameters:   data-database=<database name>
                    data-copyfield=<source>,<target>
                    data-create={Y|N}
                    data-ids=<source ID field>,<target ID field>
  28) multi_validated_field - validate field value against one of validation tables
      parameterts:  data-tableN=<table name>,<map name>[,<icon title>]+
  29) convert_measurement - convert input form input unit to output unit.
  30) touch_field   - trigger user routine that is associated with the specified field
                      data-fieldN=<field ID>
  31) downloadfile - download file to browser and move file to bakcup folder
*/
...
function exitDefaultController(calling_field)
{
  ...
  // call 'convert_measurement' class user routine
  if ( $(calling_field).hasClass('convert_measurement') ) {
    convertMeasurement(calling_field);
  }

  // call 'touch_field' class user routine
  if ( $(calling_field).hasClass('touch_field') ) {
    touchField(calling_field);
  }
...
/*
   duplicatedOcurrenceValue()

   This function checks to see the key value is defined in other occurrence of
   the field in focus.
*/

function duplicatedOcurrenceValue(calling_field, key_mnemonic, key_value)
{
  var result = false;

  var record = currentAppInterface.app_record;
  var parent_group = currentAppInterface.getGroupParent(key_mnemonic);
  var parent_id = currentAppInterface.getGroupID(calling_field);

  var grandParentGroup = null;
  if ( parent_id != '' ) {
    grandParentGroup = currentAppInterface.getGroupParent(parent_id);
    if ( typeof grandParentGroup == 'undefined' ) {
      grandParentGroup = null;
    }
  }

  var occurrence = 1;
  if ( parent_id != '' ) {
    occurrence = currentAppInterface.getCurrentOccurrence(parent_id);
  }
  else {
    occurrence = currentAppInterface.getCurrentOccurrence(key_mnemonic);
  }

  var total_occurrences = 1;
  if ( parent_id != '' ) {
    total_occurrences = record.getOccurrenceCount(parent_id.toLowerCase(), null);
  }

  var field_value = '';
  var field_parent = null;
  var field = false;
  var search_occ = 0;
  for ( var occ = 1 ; occ <= total_occurrences ; occ++ ) {
    if ( occ != occurrence ) {
      // get parent group
      if ( parent_id == '' ) {
        search_occ = occ;
        ...
        field_parent = record.getGroup(parent_id, occ, grandParentGroup);
        search_occ = 1;
      }
      field = record.getElement(key_mnemonic, search_occ, field_parent);
      if ( field == false ) {
        field_value = '';
      }
      else {
        field_value = field.text().trim();
      }
      if ( field_value == key_value ) {
        result = true;
        break;
      }
    }
  ...
  if ( dbname != "" && key_mnemonic != "" && key_value != "" ) {
    ...
    $.ajax({
      ...
      success: function (text_data) {
        ...
        var data = parser.parseFromString(text_data,"text/xml");
        if ( jQuery.isXMLDoc(data) ) {
          var duplicate_key = false;
          var xml_value = getXmlFieldValue (data, "error");
          if ( xml_value == '' || parseInt(xml_value, 10) != 0 ) {
            // s field not defined in other occurrence of curren record?
            if ( duplicatedOcurrenceValue(calling_field, key_mnemonic, key_value.trim()) ) {
              duplicate_key = true;
            }
            else {
              // if key is not found in current record, set resutlt to true
              result = true;
            }
          }
          else {
...
function escapeUrlchars(sourceField)
{
  ...
  tempString = tempString.replace (/\//g, '%2f');
...
function generateReport( calling_field )
{
  ...
  if ( ok ) {
    ...
    var url = currentAppInterface.interface_params.sessid + "/" + new_exp + "?SEARCH_N_OUTPUTFILE&CHECK_ZERO=N&NEW=Y" +
...
function setSubMenu(calling_field)
{
  ...
  // extract mnemonic of override source (data-sub-source)
  var sub_source = $(calling_field).data('sub-source');
  if ( sub_source == null ) {
    sub_source = '';
  }
  ...
  if ( main_menu_fld == '' || sub_menu_fld == '' || sub_menu == '' || main_menu_db == '' ) {
    ...
    var main_menu_value = '';
    if ( sub_source != '' ) {
      main_menu_value = $('#'+sub_source).val();
      if ( main_menu_value == null ) {
        main_menu_value = '';
      }
    }
    if (  main_menu_value == '' ) {
...
/*
  ...
  This function lets user to edit rrecord in the colorbox.
  On closing colorbox, the target ID is copied to the parent
  data entry form.
*/

function manip_record(calling_field)
{
  // extract data-database=<database name>
  var dbname = $(calling_field).data('database');
  if ( dbname == undefined ) {
    dbname = '';
  }

  // extract data-copyfield=<source>,<target>
  var copy_list = [];
  var copyfield = $(calling_field).data('copyfield');
  if ( copyfield == undefined ) {
    copyfield = '';
  }
  else {
    copy_list = copyfield.split(',');
  }

  // extract data-create={Y|N}
  var create_flag = $(calling_field).data('create');
  if ( create_flag == undefined ) {
    create_flag = 'N';
  }

  // extract data-ids=<source ID field>,<target ID field>
  var ids_list = [];
  var ids = $(calling_field).data('ids');
  if ( ids == undefined ) {
    ids = '';
  }
  else {
  ...
  if ( dbname == '' ) {
    alert ( "Database name is not specified." );
    return;
  }

  // determine popup screen size
  var dialog_width = window.innerWidth - 8; // leave spaces in left and right margin
  var dialog_height = window.innerHeight - 13; // leave spaces in top and bottom margin

  // prepare URL to add record
  var url = getCookie("HOME_SESSID");
  if ( create_flag == 'Y' ) {
    url = url + '?ADDSINGLERECORD&NEW=Y&RETURN_URL=$CLOSE_COLORBOX&DATABASE=' +
          dbname + '&RECORD_DEFAULT=Y';
    // is copyfield option defined?
    if ( copy_list.length >= 2 ) {
      // yes, set up default value of target field
      var source_id = $('#'+copy_list[0]).val();
      if ( source_id != null && source_id != '' ) {
        url = url + '&NAME1=' + copy_list[1] + '&VALUE1=' + escapeUrlchars(source_id);
      }
    }
  }
  else {
    ...
    if ( ids_list.length >= 2 ) {
      // extract source key value
      source_key = $('#'+ids_list[0]).val();
      if ( source_key == null || source_key == '' ) {
        source_key = '';
      }
      else {
        url = url + '?CHANGESINGLERECORD&NEW=Y&RETURN_URL=$CLOSE_COLORBOX&DATABASE=' +
              dbname + '&EXP=' + ids_list[1] + '%20' + escapeUrlchars(source_key);
      }
    }
    if ( source_key == '' ) {
      alert ( "Source key field is absent." ) ;
      return;
    }
  }

  // say popup data entry window
  $popup = "Y";
...
/*
  ...
  This function verifies the unit type. If unit type is one o types,
  the unit type is returned. If unit type is not found, -1 is returned.
*/

function check_unit_type ( unit_type )
{
  var  found_unit_type = -1;
  var  type_value = unit_type.toUpperCase();
  ...
  switch ( type_value ) {
   ...
   return found_unit_type;
}

/*
   unit_to_string()

   This function convert measurement untis to character string.
*/

function unit_to_string ( unit_type, unitValue, show_unit )
{
  var output_string = '';
  var result;
  ...
  // yes, show number with no fraction
  output_string = unitValue.toFixed(0).toString(10);
  }
  else {
    switch ( unit_type ) {
      case 6:   // KG
        output_string = unitValue.toFixed(3).toString(10);
        break;
      default:
        output_string = unitValue.toFixed(2).toString(10);
        break;
    }
  }
  ...
  if ( show_unit ) {
    switch ( unit_type ) {
      case 0: // M
        ...
        break;
      case 1: // CM
        output_string = output_string + 'cm';
        break;
      case 2: // FT
        output_string = output_string + 'ft';
        break;
      case 3: // IN
        output_string = output_string + 'in';
        break;
      case 4: // LB
        output_string = output_string + 'lb';
        break;
      case 5: // OZ
        output_string = output_string + 'oz';
        break;
      case 6: // KG
        output_string = output_string + 'kg';
        break;
      case 7: // G
...
/*
  ...
  This function separates the unit value and unit type form the input string.
  If unit type is found, unit value and unit type are resturned in the
  m_unti and m_value member of json result object.
*/

function extract_input_unit ( input_string )
{
  var result = {};
  var test_string = input_string.toUpperCase();
  var found_unit_type = -1;
  var kw_size = 0;
  var ix;
  ...
  // test CM unit
  kw_size = 2;
  ix = test_string.indexOf('CM');
  if ( ix > 0 && ix+kw_size == test_string.length ) {
    found_unit_type = 1;
  }
  if ( found_unit_type == -1 ) {
    // test M unit
    kw_size = 1;
    ix = test_string.indexOf('M');
    if ( ix > 0 && ix+kw_size == test_string.length ) {
      found_unit_type = 0;
    }
  }
  ...
  // Is unit type found?
  if ( found_unit_type != -1 ) {
    result.m_value = test_string.substring(0, ix).trim();
    result.m_unit = test_string.substring(ix, ix+kw_size);
  }
...
/*
  ...
  This function convert input from input unit to output unit. It expects
  the following parameters are passed in attributes of input tag.
    data-default specifies the input unit. The valid values are M|CM|FT|IN|LB|OZ|K|G.
    data-input  is the field ID of input unit. if omitted, unit specified in data-default is used.
    data-output specifies the output unit. The valid values areM|CM|FT|IN|LB|OZ|K|G.
    data-target specifies ID of target field.
    data-show={Y|N}
*/

var active_field_id = "";

function convertMeasurement(calling_field)
{
  var ok = true;
  var targetField = '';
  var inputUnit = '';
  var outputUnit = '';
  var input_unit_type = -1;
  var output_unit_type = -1;
  var output_string = '';
  var incompatible_type = false;
  var show_unit = false;
  var resultValue = 0.00;
  var empty_input = false;
  var input_string = $(calling_field).val();
  var parse_input_result = {};

  if ( typeof input_string != 'string' ) {
    input_string = '';
  }
  else {
    parse_input_result = extract_input_unit ( input_string );
    if ( 'm_value' in parse_input_result ) {
      input_string = parse_input_result.m_value;
    }
  }

  // extract data-show option
  targetField = $(calling_field).data('show');
  if ( typeof targetField == 'string' ) {
    if ( targetField.toUpperCase() == 'Y' ) {
      show_unit = true;
    }
  }

  // extract taget field ID
  targetField = $(calling_field).data('target');
  if ( targetField === undefined || targetField == null ) {
    alert ( "Target field ID is not defined." );
    targetField = '';
    ok = false;
  }
  else {
    if ( active_field_id != '' && active_field_id == targetField ) {
      // if recursive call, do nothing and exit function
      return;
    }

    if ( 'm_unit' in parse_input_result ) {
      inputUnit = parse_input_result.m_unit;
    }
    else {
      // extract input unit (data-input)
      inputUnit = $(calling_field).data('input');
      if ( inputUnit != undefined && inputUnit != '' ) {
        inputUnit = $('#'+inputUnit).val();
      }
    }
    if ( inputUnit === undefined || inputUnit == '' ) {
      // extract default input unit (data-default)
      inputUnit = $(calling_field).data('default');
    }
    if ( inputUnit === undefined || inputUnit == '' ) {
      alert ( "Input unit is not defined." );
      inputUnit = '';
      ok = false;
    }
    else {
      input_unit_type = check_unit_type ( inputUnit );

      // extract output unit (data-output)
      outputUnit = $(calling_field).data('output');
      if ( outputUnit === undefined || outputUnit == '' ) {
        alert ( "Output unit is not defined." );
        outputUnit = '';
        ok = false;
      }
      else {
        output_unit_type = check_unit_type ( outputUnit );
      }

      if ( input_unit_type == -1 || output_unit_type == -1 ) {
        if ( input_unit_type == -1 ) {
          alert ( "Input unit is unknown." );
        }
        else {
          alert ( "Output unit is unknown." );
        }
        ok = false;
      }
      else {
        // is input unit compatible with output unit?
        if ( (input_unit_type >= 0 && input_unit_type <= 3)
        &&   (output_unit_type < 0 || output_unit_type > 3) ) {
          // incompatible length measurement
          ok = false;
        }
        else {
          if ( (input_unit_type >= 4 && input_unit_type <= 7)
          &&   (output_unit_type < 4 || output_unit_type > 7) ) {
            // incompatible weight measurement
            ok = false;
          }
        }
  ...
  if ( ok ) {
    ...
    if ( input_string == '' ) {
      ...
      resultValue = Number(input_string);
      if ( resultValue == NaN ) {
        ok = false;
      }
    }
    ...
    if ( !ok ) {
      alert ( "Input is invalid numeric string." );
    }
  }

  // if input unit is equal to output unit, do nothing
  if ( ok && !empty_input && input_unit_type != output_unit_type ) {
    // no, convert meseasurement unit
    if ( ok ) {
      switch ( input_unit_type ) {
      case 0:   /* M */
        resultValue *= 100.00;
        if ( output_unit_type == 1 ) { // CM
          break;
        }
      case 1:   /* CM */
        switch ( output_unit_type ) {
          case 0:    /* M */
            ...
            break;
          case 2:   /* FT */
            resultValue = (resultValue * 0.393701) / 12.00;
            break;
          case 3:   /* IN */
            resultValue *= 0.393701;
            break;
          default:
            incompatible_type = true;
            break;
      ...
      case 3:    /* IN */
        switch ( output_unit_type ) {
          case 2:   /* FT */
            ...
            break;
          case 0:   /* M */
            resultValue = (resultValue * 2.54) / 100.00;
            break;
          case 1:   /* CM */
            resultValue *= 2.54;
            break;
      ...
      case 5:   /* OZ */
        switch ( output_unit_type ) {
          case 4:   /* LB */
            ...
            break;
          case 6:   /* KG */
            resultValue = (resultValue * 28.3495) / 100.00;
            break;
          case 7:   /* G */
            resultValue *= 28.3495;
            break;
      ...
      case 7:    /* G */
        switch ( output_unit_type ) {
          case 6:   /* KG */
            ...
            break;
          case 4:   /* LB */
            resultValue = (resultValue * 0.035274) / 16.00;
            break;
          case 5:   /* OZ */
            resultValue *= 0.035274;
            break;
          default:
            incompatible_type = true;
            break;
  ...
  if ( incompatible_type ) {
    alert ( "Input unit is incompatible with output unit." );
    ok = false;
  }

  if ( ok ) {
    // set flag to prevent recursive call
    active_field_id = $(calling_field).attr("id");
    if ( typeof active_field_id != 'string' ) {
      active_field_id = "";
    }
    ...
    output_string = unit_to_string ( output_unit_type, resultValue, show_unit );
      if ( output_string == '' ) {
        alert ( "Unable to convert " + resultValue + " to string" );
        ok = false;
      }
    }

    if ( ok ) {
      // save converted data in target field
      $('#'+targetField).val(output_string).change();
    }
...
/*
  ...
  This function tirggers the use rroutine that is assoicated with the specified field(s)..
    data-fieldN - <field ID>.  one or more data-fieldN options are allowed. N is
  numbered form 1.
*/

function touchField(calling_field)
{
  var   ix;
  var   field_id;
  var   option_value;
  ...
  while ( true ) {
    ix++;
    field_id = "field" + ix;
    option_value = $(calling_field).data(field_id);
    if ( option_value === undefined || option_value == '' ) {
      // if no mroe data-fieldN option, exit loop
      break;
    }
