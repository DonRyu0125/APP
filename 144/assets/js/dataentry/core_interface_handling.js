...
function ApplicationInterface(record, caller_params) {
  ...
  this.populateField = function (field_id, occurrence, jquery_field, richTextFlag) {
    ...
    var data_edit = $(form_field_id).data('edit');
    if ( typeof data_edit != 'string' ) {
      data_edit = 'N';
    }
    else {
      data_edit = data_edit.toUpperCase();
    }
  ...
  this.handleImages = function () {
    ...
    if ($(params.record_image).length > 0) {
      if (typeof record.getPrimaryImage == 'function' ) {
        h_image = record.getPrimaryImage();
        if (typeof h_image == 'undefined' ) {
          // console.log ( "primary_image is undefined." );
          return false;
        }
      }
    }
    else {
      // console.log ( "primary_image field is not found." );
      return false;
    }

    if ( h_image != false ) {
      // populate first image
      var  image_field = h_image.image;
      if (image_field) {
    image_field.text(MiniissVdToIisVd(params, image_field.text()));

        $(params.record_image).attr('src',  image_field.text());
        ...
        if ($(params.record_image2).length > 0) {
          if ( typeof record.getSecondImage == 'function' ) {
            h_image = record.getSecondImage();
            if (typeof h_image == 'undefined' ) {
              // console.log ( "second_image is undefined" );
              return false;
            }
          }
          if ( h_image != false ) {
            // populate second image
            image_field = h_image.image;
      image_field.text(MiniissVdToIisVd(params, image_field.text()));

            $(params.record_image2).attr('src',  image_field.text());
  ...
  */
  ...
  this.loadExternalLink = function (calling_field) {
    ...
    caller_value = MiniissVdToIisVd(params, caller_value);
  ...
  */
  ...
  this.uploadFile = function (calling_field) {
    ...
    var media_type = $(calling_field).data('media-type');
    ...
    // RL-2020-09-29
    url = params.sessid + "?get&file=" + UPLOAD_HTML_FILE + "&parm1=" + accession_number + "&parm2=" + params.image_virtual_directory + "&parm3=" + media_type;
    // RL-2021-05-31
    var dialog_width = window.innerWidth - 8; // leave spaces in left and right margin
    var dialog_height = window.innerHeight - 13;  // leave spaces in top and bottom margin
  ...
  this.downloadFile = function (calling_field) {
    ...
    if (file_url.length == 0) {
      return false;
    } else if (file_url.toUpperCase().indexOf(DEFAULT_MEDIA_PATH) >= 0) {
  ...
  this.updateImages = function (calling_field) {
    ...
    if ($(calling_field).parents('.contains_image').length > 0) {
      ...
      if (image_src.length > 0) {
        image_src = MiniissVdToIisVd(params, image_src);
...
// replace MINISIS VD diretory path with web path
function MiniissVdToIisVd(params, MinisisVd)
{
  var result = MinisisVd;

  if (MinisisVd.indexOf(params.image_virtual_directory) > -1) {
    result = MinisisVd.replace(params.image_virtual_directory, params.image_url_path);
  }
  else {
    if (MinisisVd.indexOf(params.image_virtual_directory.toLowerCase()) > -1) {
      result = MinisisVd.replace(params.image_virtual_directory.toLowerCase(), params.image_url_path);
    }
  }
