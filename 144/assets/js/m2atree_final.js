...
const minisis_online_default_icon =  ["/144/icons/minisis_default_database_00_01.png", // Database (Archive, Museum, Library)
                                      ...
                                      "/144/icons/minisis_default_sub_collection_02.png", // Sous-fonds, sub-collections
                                      ...
                                      "/144/icons/minisis_default_component_09.png"]; // Component, Diffusion Material
...
function tree_clear_treenode ( tree_node, level )
{
  ...
  if ( num_children > 0 ) {
    ...
    for ( i = num_children - 1 ; i >= 0 ; i-- ) {
      tree_clear_treenode ( tree_node.m_child_nodes[i], level + 1 );
...
function pagination_handler ( id ) {
  ...
  if ( i >= 0 ) {
    ...
    if ( parent_tree_node == null ) {
      ...
      tree_clear_treenode ( parent_tree_node, 0 );
                          ...
                          target_position )
{
  ...
  if ( m2a_tree == null ) {
    ...
    if ( parent_node == null ) {
      ...
      if ( parent_id == TOP_ID && (add_child_node || title_updated) ) {
        var jstree_node = $(jtree_variable).jstree(true).get_node(parent_id);
        if ( jstree_node != null ) {
          m2a_tree.setFocus ( TOP_ID );
          var link = $('#dba_tree').find('a').attr('href');
          if ( link != null ) {
...
function TreeControl ( parm_file, location_id ) {
  ...
  // check readonly tree flag
  var h_string = $(jtree_variable).data('readonly');
  if ( typeof h_string == 'string' && h_string.toUpperCase() == 'Y' ) {
  ...
  this.changeRecord = function (obj) {
    ...
    if ( node_id !== TOP_ID && node_id !== ROOT_ID ) {
      ...
      if ( tree_node ) {
        ...
        if ( go_ahead ) {
          if ( m2a_tree.m_readonly ) {
            // prepare REDONLYFORM command
            url = this.m_home_sessid + "?READONLYFORM" +
              "&EXP=SISN+" + tree_node.m_id;
          }
          else {
        ...
        if ( target_tree_node != null ) {
          ...
          if ( source_parent_node_id !== target_parent_node_id || paste_before ) {
            ...
            &&   ajax_result.m_result_code != 4303 ) { // refresh source and target parent tree node
              ...
              if ( target_parent_node_id == TOP_ID || target_parent_node_id == ROOT_ID ) {
                // refresh entire page
                this.setFocus ( desc_sisn );
                var link = $('#dba_tree').find('a').attr('href');
                if ( link != null ) {
                  window.location = link;
                }
  ...
  this.treeMenu = function (node) {
    ...
    if ( node.id.indexOf(PAGE_SEPARATOR) >= 0 ) {
      ...
      if ( !m2a_tree.m_readonly ) {
        ...
        if ( !root_menu ) {
          menu_list.changeItem = {};
          menu_list.changeItem.label = "View Record by Form";
          menu_list.changeItem.action = function (obj) { m2a_tree.changeRecord(node); };
          menu_list.changeItem.icon = "fa fa-eye";
          menu_list.changeItem._disabled = false;
        }
      }
      menu_list.refreshItem = {};
...
$(document).ready( function() {
  ...
  if ( m2a_tree == null ) {
    alert ( "Unable to create description tree hierarchy." );
  }
  else {
