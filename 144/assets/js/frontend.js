...
var toggleNavIcon = function () {
  if ( $("body").attr("data-database") == DESCRIPTION_DBNAME ) {
    ...
    if ( typeof tree_direct_edit != 'undefined' && !tree_direct_edit ) { // RL-20230330
...
// This function extracts home Session ID form the borwser cookie.
var getHomeSessId = function () {
  var homeSessId = getCookie ( 'HOME_SESSID' );
  if ( homeSessId == '' ) {
    homeSessId = 'HOME_SESSID';
  }
  return homeSessId;
};
    ...
    if (typeof type != 'undefined' && type == "grid") {
      ...
      if ( !search_results_metadata.no_heading ) { // RL-20220311
        if ( search_results_metadata.own_heading != '' ) {
          ...
          var result = setTableHeadingLine ( database );
          if ( data.length == 0 ) {
            html.append(generateNoResultsBody(result.numColums));
          }
          else {
            html.append($(result.headingLine));
          }
        }
      }
          ...
          for (let i = 0; i < resultObject.length; i++) {
            if (resultObject[i].children) {
              ...
              if ( tr_class != null && tr_class != "sum-heading" ) {  // RL-20220131
                ...
                // RL-20211201
                var image_tag = img_tag_header + image_path + '" alt="image" onError="this.onerror=null;this.src=\'/144/assets/img/image-placeholder.png\'">';
  ...
  *****************************************************/
  ...
  var handle3rdReport = function (database, report) { // RL-20220311
    if ( current_page_link != '' ) {
      ...
      alert ( 'Cureent page link is undefined.' );
    }
  }

  /*
     handleSearchResults - main line start here
  */
        ...
        if ( select_group != null && select_object.length > 0 ) {
          ...
          $(option).each(function() {
            if ( $(this).is(':selected') ) {
              ...
              if ( group_occ == undefined ) {
                group_occ = '1';
              }

              fieldValue = $(select_object).val();

              // save data-select option and data-occ option in xml record
              var xml_record_node = $(xml_record).find('record').first();
              var tempstring = '<_fieldInfo></_fieldInfo>';
              $(tempstring).appendTo(xml_record_node);
              var occ_info = $(xml_record_node).find('_fieldInfo').first();
              tempstring = '<fieldName>' + select_group + '</fieldName><fieldValue>' + fieldValue + '</fieldValue>';
...
var generateDetailedReport = function (record_xml) {
  ...
  $(record_xml).find('*').each(function () {
    if ($(this).children().length === 0) {
      ...
      var text_value = $(this).text();   // RL-20230314
      var element_value = $('<dd>' + text_value + '</dt>');  // RL-20230314
      var html = $('<li/>');
      var container = $('<dl/>');
      ...
      if ( text_value.length > 0 ) { // RL-20230314
...
var handleDetailedRecordDisplay = function (record_xml) {
  ...
  // RL-20230305
  var preformat_record = null;
  var has_preformat_div = false;
  if ( $(record_xml).find('pre').length > 0 ) {
    has_preformat_div = true;
    preformat_record = $('<div id="detail_record_display"/>');
    preformat_record.append($(record_xml).find('pre'));
  }
  ...
  var ro_view_link = $(record_xml).find('readonly_view_link').text();
  ...
  if ( delete_link != null && delete_link != '' ) {  // RL-20230305
    $('li#dba_delete').find('a').removeClass('disabled').attr('data-delete-url', delete_link);
  }
  if ( ro_view_link != null && ro_view_link != '' ) {  // RL-20230527
    $('li#dba_view').find('a').removeClass('disabled').attr('href', ro_view_link);
  }
  ...
  // RL-20230206
  var  print_a_tag = $('li#dba_print a');
  var  reports_a_tag = $('li#dba_reports a'); // RL-202302
  if ( typeof print_a_tag == 'undefined' ) {
  print_a_tag = reports_a_tag;
  }
  else {
  $('li#dba_reports a').addClass('disabled');
  }
  if ($(print_a_tag).hasClass('disabled')) { // RL-20230206
    $(print_a_tag).removeClass('disabled');  // RL-20230206
  }
  ...
  if ( $(print_a_tag).length > 0 ) {
    ...
    print_a_tag.on('click', function (e) {
      ...
      if ( typeof printJS == 'function' ) {
        var dbname = $('body').data('database');
        var options = { printable: detail_record_id, type: "html", documentTitle: doc_title};
        printJS(options);
      }
    });
  }
  ...
  // RL-20230305
  var html_codes = null;
  if ( has_preformat_div ) {
    html_codes = preformat_record;
  }
  else {
    html_codes = generateDetailedReport(record_xml);
  }

  $('section#body div.container div.results_container').html(html_codes);  // RL-20230305
};
...
var handleReports = function (e, searchresult) {
  ...
  if ( typeof searchresult != 'undefined' && searchresult != '' ) {
    ...
    $('#report_chooser a.button').on('click', function () {
      // extract report name and type - RL-20230206
      var options = $('#report_chooser select').val().split(',');
      var url = searchresult + '?OUTPUTFILE&CHECK_ZERO=N&REPORT=' + options[0] + '&USE_BOOKMARK=Y&DOWNLOADPAGE=' + PDF_DOWNLOAD_PAGE;
      if ( options.length > 1  ) {
        url = url + "&EXTENSION=" + options[1];
      }
...
var confirmSearch = function(caller_title, zero_hit_text, question_text) {
  ...
  if ( customized_report_parms.always != null
  &&   customized_report_parms.always.toUpperCase() == 'Y' ) {
    always = true;
  }
  ...
  if ( always && has_customized_query ) {
    new_search = true;
  }
  else {
...
var handlePlanMove = function(e) {
  ...
  if (!search_database ) {
    var url = getHomeSessId() + "?get&file=[m2ademo]dataentry\\globaladd-e.html";
...
var browseColorbox = function (caller) {
  ...
  var url = get_sessid() + '/FIRST?INDEXLIST&WINDOW=' + field_name + '&KEYNAME=' + field_name + '&TITLE=' + field_title;
  ...
  var colorbox_params = {
    ...
    'onClosed': function () {
      if (typeof tmp_val === 'undefined') {
        ...
        tmp_val = undefined;  // MH-2023-02-16: Clear vakue of tmp_val
      }
    }
  };
...
/**
 *
 *  This function extracts cookie value from browser session.
 */
...
// This function seta the cooke value of browser session.
...
var callMwiApi = function (url) {
  ...
  $.ajax({
    ...
    var parser = new DOMParser();
        result_data = parser.parseFromString(data,"text/xml");
      }
    },
    error: function (xhr, status, error) {
      result_data = "";
...
var save_hits = function () {
  ...
  if ( input != "" ) {
    var dbname = $('body').data('database');
    var url = getHomeSessId() + "/0?SAVEHITS&DIRECTORY=" + HIT_STRATEGY_PATH + "USERDATA&HITS_NAME=" + input;
    if ( typeof dbname != 'undefined' && dbname != "" ) {
      url = url + "&DATABASE=" + dbname;
    }
...
var save_strategy = function () {
  ...
  if ( input != "" ) {
    var url = getHomeSessId() + "/0?SAVESTRATEGY&DIRECTORY=" + HIT_STRATEGY_PATH + "USERDATA&STRATEGY_NAME=" + input;
    var result = callMwiApi(url);
    if ( jQuery.isXMLDoc( result ) ) {
      var ecode_nodes = result.getElementsByTagName("error_code");
      if ( ecode_nodes.length > 0 ) {
    ...
    if ( report_params == null ) {
      ...
      if ( customized_report_parms != null ) {
        ...
        ||   customized_report_parms.report != "" ) {
          // RL-20230209
          var report_option = customized_report_parms.report.split(',');
          var searchResultInfo = lastSearchHits();
          var url = getHomeSessId() + "/" + searchResultInfo.last_expno +
                    "?OUTPUTFILE&CHECK_ZERO=N&REPORT=" + report_option[0]  +
                    "&DOWNLOADPAGE=[m2ademo]pdf.html&USE_BOOKMARK=Y"; /* RL-2021-03-09 */
          // RL-20230209
          if ( report_option.length > 1  ) {
            url = url + "&EXTENSION=" + report_option[1];
          }
          $.colorbox({
            href: url
          });
...
function removeModalLinks(report_page) {
  ...
  if ( dba_link != null && $(dba_link).hasClass('disabled') ) {
    $(dba_link).removeClass('disabled');
  }
...
$(document).ready(function () {
  ...
  */
  ...
  // "span#userName").text(getUserName());
  $('span#userName').text(getUserName());  // M3-2020-12-14 - M3 codes
